...
async function installPackage(runtime, pkgType, pkgName, options) {
  ...
  try {
    switch (pkgType) {
    case 'apt':
      await new AptTask(options.name, runtime, options.codeUri, pkgName, options.local, ctx).run();
      break;
    case 'pip':
      await new PipTask(options.name, runtime, options.codeUri, pkgName, options.local, ctx).run();
      break;
    case 'module':
      // TODO
      break;
...
async function installFromYaml(file) {
  ...
  try {
    for (const t of funModule.tasks) {
      if (t.type === 'pip') {
        const pipTask = new PipTask(t.attrs.name, runtime, codeUri, t.attrs.pip, t.attrs.local, ctx);
        await pipTask.run();
      } else if (t.type === 'apt') {
        const aptTask = new AptTask(t.attrs.name, runtime, codeUri, t.attrs.apt, t.attrs.local, ctx);
        await aptTask.run();
      } else if (t.type === 'shell') {
        const shellTask = new ShellTask(t.attrs.name, runtime, codeUri, t.attrs.shell, ctx);
        await shellTask.run();
      } else {
